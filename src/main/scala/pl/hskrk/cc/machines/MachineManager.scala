...
class MachineManager extends Actor {
  ...
  var machines = {
    ...
    List(
      Machine(MachineId(1),"Szlifierka", now.minusDays(1)),
      Machine(MachineId(2),"Tokarka", now.minusDays(2)),
      Machine(MachineId(3),"Lutownica", now.minusDays(3)),
      Machine(MachineId(4),"Kompresor", now.minusDays(4)),
      Machine(MachineId(5),"Drukarka 3D", now.minusDays(5))
    )
  }
